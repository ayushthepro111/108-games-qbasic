...
COLOR 14
LOCATE 21, 8: PRINTF "(C)opyright 98"
KP = INPUT$(1)
IF KP = CHR$(27) THEN
END
END IF
...
FUNCTION EDITOR
DIM W(100, 50), V(100, 50)
MAXIMUM.X = 64 - 1: MAXIMUM.Y = 48 - 1
H = 62 / 2: V = 46 / 2
'build maze

CLS : Q = 0: Z = 0: X = INT(RND * H) + 1
     FOR I = 1 TO H
       IF I = X THEN PRINT "лx";  ELSE PRINT "лл";
1022 NEXT I
     PRINT "л"
     C = 1
     W(X, 1) = C
     ...
     R = X
     S = 1
     LOCATE 25, 2: PRINT "CREATING MAZE";
     LOCATE 2, 1
     GOTO 1042
1030 IF R <> H THEN 1038
     IF S <> V THEN 1036
     R = 1: S = 1: GOTO 1040
1036 R = 1: S = S + 1: GOTO 1040
1038 R = R + 1
1040 IF W(R, S) = 0 THEN 1030
1042 IF R - 1 = 0 THEN 1108
     IF W(R - 1, S) <> 0 THEN 1108
     IF S - 1 = 0 THEN 1074
     IF W(R, S - 1) <> 0 THEN 1074
     IF R = H THEN 1058
     IF W(R + 1, S) <> 0 THEN 1058
     X = INT(RND * 3) + 1
     ON X GOTO 1176, 1184, 1192
1058 IF S <> V THEN 1064
     IF Z = 1 THEN 1070
     Q = 1: GOTO 1066
1064 IF W(R, S + 1) <> 0 THEN 1070
1066 X = INT(RND * 3) + 1: ON X GOTO 1176, 1184, 1206
1070 X = INT(RND * 2) + 1: ON X GOTO 1176, 1184
1074 IF R = H THEN 1094
     IF W(R + 1, S) <> 0 THEN 1094
     IF S <> V THEN 1084
     IF Z = 1 THEN 1090
     Q = 1: GOTO 1086
1084 IF W(R, S + 1) <> 0 THEN 1090
1086 X = INT(RND * 3) + 1: ON X GOTO 1176, 1192, 1206
1090 X = INT(RND * 2) + 1: ON X GOTO 1176, 1192
1094 IF S <> V THEN 1100
     IF Z = 1 THEN 1106
     Q = 1: GOTO 1102
1100 IF W(R, S + 1) <> 0 THEN 1106
1102 X = INT(RND * 2) + 1: ON X GOTO 1176, 1206
1106 GOTO 1176

1108 IF S - 1 = 0 THEN 1146
     IF W(R, S - 1) <> 0 THEN 1146
     IF R = H THEN 1132
     IF W(R + 1, S) <> 0 THEN 1132
     IF S <> V THEN 1122
     IF Z = 1 THEN 1128
     Q = 1: GOTO 1124
1122 IF W(R, S + 1) <> 0 THEN 1128
1124 X = INT(RND * 3) + 1: ON X GOTO 1184, 1192, 1206
1128 X = INT(RND * 2) + 1: ON X GOTO 1184, 1192
...
1146 IF R = H THEN 1164
     ...
     IF Z = 1 THEN 1162
     Q = 1: GOTO 1186
1156 IF W(R, S + 1) <> 0 THEN 1162
     X = INT(RND * 2) + 1: ON X GOTO 1192, 1206
1162 GOTO 1192
1164 IF S <> V THEN 1170
     IF Z = 1 THEN 1174
     Q = 1: GOTO 1172
1170 IF W(R, S + 1) <> 0 THEN 1174
1172 GOTO 1206
1174 GOTO 1226
1176 W(R - 1, S) = C: C = C + 1
     V(R - 1, S) = 2: R = R - 1
     IF C = 1 + (H * V) THEN 1228
     Q = 0: GOTO 1042
1184 W(R, S - 1) = C
1186 C = C + 1: V(R, S - 1) = 1: S = S - 1
     IF C = 1 + (H * V) THEN 1228
     Q = 0: GOTO 1042
1192 W(R + 1, S) = C: C = C + 1
     IF V(R, S) = 0 THEN 1198
     V(R, S) = 3: GOTO 1200
1198 V(R, S) = 2
1200 R = R + 1
     IF C = 1 + (H * V) THEN 1228
     GOTO 1108
1206 IF Q = 1 THEN 1218
     W(R, S + 1) = C: C = C + 1: IF V(R, S) = 0 THEN 1212
     V(R, S) = 3: GOTO 1214
1212 V(R, S) = 1
1214 S = S + 1: IF C = 1 + (H * V) THEN 1228
     GOTO 1042
1218 Z = 1: IF V(R, S) = 0 THEN 1224
     V(R, S) = 3: Q = 0: GOTO 1226
1224 V(R, S) = 1: Q = 0: R = 1: S = 1: GOTO 1040
1226 GOTO 1030

1228 FOR J = 1 TO V
       PRINT "л";
       FOR I = 1 TO H
         IF V(I, J) < 2 THEN PRINT " л";  ELSE PRINT "  ";
       NEXT I
       PRINT
       FOR I = 1 TO H
         IF V(I, J) = 0 OR V(I, J) = 2 THEN PRINT "лл";  ELSE PRINT "л ";
       NEXT I
       PRINT "л"
     NEXT J
...
FOR X = 1 TO MAXIMUM.X
FOR Y = 1 TO MAXIMUM.Y
IF SCREEN(Y, X) = 32 THEN EDIT.LEVEL(X, Y) = 0 ELSE EDIT.LEVEL(X, Y) = 14
IF SCREEN(Y, X) = ASC("x") THEN EDIT.LEVEL(X, Y) = -50
IF EDIT.LEVEL(X, Y) = 0 THEN
IF INT(RND * 20) + 1 <> 1 THEN EDIT.LEVEL(X, Y) = 11 ELSE EDIT.LEVEL(X, Y) = 12
END IF
NEXT Y
NEXT X
CLS
FOR X = 1 TO MAXIMUM.X
IF EDIT.LEVEL(X, MAXIMUM.Y) <> 14 THEN EDIT.LEVEL(X, MAXIMUM.Y) = 14: EDIT.LEVEL(X, MAXIMUM.Y - 1) = 8
IF EDIT.LEVEL(X, 1) = -50 THEN EDIT.LEVEL(X, 1) = 14: EDIT.LEVEL(X, 2) = 3
NEXT X
X1 = MAXIMUM.X / 3
X2 = (MAXIMUM.X / 3) * 2
Y1 = MAXIMUM.Y / 3
...
FOR X = X1 TO X2
FOR Y = Y1 TO Y2
IF EDIT.LEVEL(X, Y) <> 14 THEN
IF INT(RND * 22) + 1 = 1 THEN
IF INT(RND * 2) + 1 = 1 THEN EDIT.LEVEL(X, Y) = 10 ELSE EDIT.LEVEL(X, Y) = 9
END IF
END IF
NEXT Y
NEXT X
...
END SELECT
CASE ELSE
IF EDIT.LEVEL(X, Y) = 11 THEN
W$ = TYPE$(EDIT.LEVEL(X, Y - 1)) + TYPE$(EDIT.LEVEL(X, Y + 1)) + TYPE$(EDIT.LEVEL(X - 1, Y)) + TYPE$(EDIT.LEVEL(X + 1, Y))
SELECT CASE W$
CASE "OOOO": EDIT.LEVEL(X, Y) = 0
CASE " OOO": EDIT.LEVEL(X, Y) = 29
CASE "O OO": EDIT.LEVEL(X, Y) = 29
CASE "OO O": EDIT.LEVEL(X, Y) = 29
CASE "OOO ": EDIT.LEVEL(X, Y) = 29
END SELECT
END IF
END SELECT
