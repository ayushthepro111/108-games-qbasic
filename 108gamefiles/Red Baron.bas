'
' Red Baron
' By CyberTouch Productions
' Coded By:AARON JOHNSON
...
KeyboardInput:                                      
  A$ = INKEY$
    IF A$ = CHR$(0) + CHR$(77) THEN
       CALL MoveRight(X%): GOSUB Upd
       END IF
    IF A$ = CHR$(0) + CHR$(75) THEN
       CALL MoveLeft(X%): GOSUB Upd
       END IF
    IF A$ = CHR$(0) + CHR$(72) THEN
       CALL MoveUp(Y%): GOSUB Upd
       END IF
    IF A$ = CHR$(0) + CHR$(80) THEN
       CALL MoveDown(Y%): GOSUB Upd
       END IF
    IF A$ = CHR$(0) + CHR$(73) THEN
       CALL UpRight(X%, Y%): GOSUB Upd
       END IF
    IF A$ = CHR$(0) + CHR$(71) THEN
       CALL UpLeft(X%, Y%): GOSUB Upd
       END IF
    IF A$ = CHR$(0) + CHR$(79) THEN
       CALL DownLeft(X%, Y%): GOSUB Upd
       END IF
    IF A$ = CHR$(0) + CHR$(81) THEN
       CALL DownRight(X%, Y%): GOSUB Upd
       END IF
...
'(continued)                                        
'End of file                                        
'IMPORTed with ProSlice by Thomas Lipschultz        
...
'(continued)                                        
  ...
  LINE (216, 25)-(265, 25), 12
  LOCATE 3, 28: COLOR 14: PRINT "CyberTouch"
END SUB
